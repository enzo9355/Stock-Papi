...
本機資料根目錄固定為 `D:\StockPapiData`。年度交易日曆預設位置：

```text
D:\StockPapiData\publish\calendars\v1\TW-<year>.json
```
...
.\scripts\run_tw_post_close_pipeline.ps1 -DataRoot D:\StockPapiData

# 盤前：只做 overnight overlay
.\scripts\run_tw_pre_market_pipeline.ps1 -DataRoot D:\StockPapiData

# 背景完整回測，每次最多 25 檔；重跑會從 checkpoint 繼續
.\scripts\run_full_backtest.ps1 -DataRoot D:\StockPapiData -MaxItems 25

# 美股獨立批次
.\scripts\run_us_daily.ps1 -DataRoot D:\StockPapiData

# 模型週報；亦可手動觸發，不依賴特定星期
.\scripts\run_weekly_model.ps1 -DataRoot D:\StockPapiData

# 狀態
python -m stock_papi.batch.cli status --root D:\StockPapiData
```
...
python -m reporting.backfill `
  ...
  --calendar-artifact D:\StockPapiData\publish\calendars\v1\TW-2026.json
```
...
```powershell
.\scripts\install_pipeline_tasks.ps1 -DataRoot D:\StockPapiData -WhatIf
```

確認 wrapper、D 槽 ACL、calendar、promoted backtest、GCP 與 LINE 設定後才正式安裝：

```powershell
.\scripts\install_pipeline_tasks.ps1 -DataRoot D:\StockPapiData
```

新工作：

- `StockPapi-TW-PostClose` 17:10
- `StockPapi-TW-PreMarket` 07:30
- `StockPapi-FullBacktest` 22:30
- `StockPapi-US-Daily` 05:30
- `StockPapi-WeeklyModel` 週六 18:00（安裝時可用 `-WeeklyDay Sunday`）
- `StockPapi-ReportUploadRecovery` 09:35

安裝器不刪除舊 `StockPapi-LocalQuant`／`StockPapi-QuantUpload`。先讓新流程只產生本地 candidate，比對數日後才停用舊工作；本 repository 變更不代表已完成 production cutover。
