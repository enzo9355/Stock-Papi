...
- Windows 排程 `StockPapi-LocalQuant` 每日台北時間 05:30 啟動，09:20 停止領取新股票，09:30 強制結束；錯過時段不在白天補跑。
- 每晚持續處理台股 universe，直到全市場完成或 09:20 停止領取新工作；逐檔產生 730 日特徵、五日預測、回測與 gzip JSON 快照，未完成時隔日從 checkpoint 續跑。
...
- 安裝或重新驗證：`powershell -ExecutionPolicy Bypass -File .\scripts\install_local_quant_task.ps1`。
