...
if ($AllowDegradedBootstrap) { $QuantArguments += '--allow-degraded-bootstrap' }
...
$CandidateArguments = @('-m', 'stock_papi.batch.daily_products_cli', 'build', '--root', $DataRoot, '--source-market-date', $Manifest.market_as_of, '--source-manifest', "quant/v1/$ManifestRelative", '--source-manifest-sha256', $Latest.manifest_sha256, '--calendar-artifact', $CalendarPath)
if ($AllowDegradedBootstrap) { $CandidateArguments += '--allow-degraded-bootstrap' }
...
& $PythonExe -m stock_papi.batch.daily_products_cli promote --root $DataRoot --candidate $Candidate.candidate_path
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& (Join-Path $PSScriptRoot 'upload_local_quant.ps1') -DataRoot $DataRoot -RequireReportV2 -RequireDashboard
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
