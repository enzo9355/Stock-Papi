...
class DailyIndustryReportGenerator:
    ...
    def _build_pdf(self, report: DailyIndustryReport, target: Path) -> None:
        ...
        def kpi_cards(items):
            cards = []
            for label, value, formatter in items:
                if value is None:
                    continue
                formatted_value = formatter(value)
                card = Table(
                    [[paragraph(label, small)], [paragraph(formatted_value, kpi)]],
                    colWidths=[40 * mm],
                )
                ...
                cards.append(card)
            if not cards:
                return [paragraph("今日產業樣本尚未完成，暫不提供產業關鍵指標。", small)]
            padded_cards = list(cards)
            while len(padded_cards) % 4 != 0:
                padded_cards.append("")
            rows = [padded_cards[index:index + 4] for index in range(0, len(padded_cards), 4)]
        ...
        has_market_data = any(v is not None for v in market.returns.values()) or manifest.coverage is not None
        if has_market_data:
            story += section("市場與資料品質")
            story += kpi_cards([
                ("市場 1 日報酬", market.returns.get(1), lambda v: _pct(v)),
                ("市場 5 日報酬", market.returns.get(5), lambda v: _pct(v)),
                ("市場 20 日報酬", market.returns.get(20), lambda v: _pct(v)),
                ("市場 60 日報酬", market.returns.get(60), lambda v: _pct(v)),
                ("市場波動率", market.volatility_20d, lambda v: _pct(v)),
                ("上漲／下跌家數", (market.advancing_count, market.declining_count) if market.advancing_count is not None and market.declining_count is not None else None, lambda v: f"{v[0]}／{v[1]}"),
                ("20 日新高／新低", (market.new_high_20d_count, market.new_low_20d_count) if market.new_high_20d_count is not None and market.new_low_20d_count is not None else None, lambda v: f"{v[0]}／{v[1]}"),
                ("量能相對 20 日平均", market.average_volume_ratio, lambda v: f"{v:.2f} 倍"),
            ])
            story += [Spacer(1, 3 * mm)]
        ...
        # 3. 產業輪動
        has_rotation = any(item.coverage >= self.config.min_industry_coverage and item.relative_return_5d is not None and item.relative_return_20d is not None for item in report.industries)
        if has_rotation:
            story += section("產業輪動")
        ...
        if quality.pooled_oos_samples is not None and quality.pooled_oos_samples > 0:
            story += section("整體模型品質")
            story += kpi_cards([
                ("pooled OOS 樣本數", quality.pooled_oos_samples, lambda v: str(v)),
                ("歷史方向準確率", quality.direction_accuracy, lambda v: _pct(v)),
                ("Brier Score", quality.brier_score, lambda v: _number(v, 4)),
                ("高分訊號實際上漲率", quality.high_score_win_rate, lambda v: _pct(v)),
            ])
        ...
        if report.backtests:
            ...
            story += [paragraph(
                "成本口徑：每次五日完整持有扣除 0.585%；產業買進持有與市場基準均未扣成本。低樣本 Sharpe 與策略交易勝率僅供參考，不用於排序推薦產業。",
                small,
            )]
            ...
            risk_rows = [["產業", "最大回撤", "Sharpe", "策略勝率", "樣本警示"]]
